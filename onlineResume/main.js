$(function(){
	...
	// 		'direction' : 'vertical'
	// });
    // var bt = $('#bubble1').css('bottom').split('p')[0];
    // alert(bt)
    
  //   $('.headImg, .selfInfoCont').each(function(){
  ...
  //   })
   //小鱼游动
   var eduWrap = $('.eduWrap');
   var moveFish = $('.moveFish');
   moveFish.fadeIn(500,function(){
       $('#contBox1').fadeIn(500,function(){
       	   moveFish.fadeOut('slow');
       	   var steps = [{top:'60px',left:'308px'},{top:'155px',left:'348px'},{top:'240px',left:'398px'}];
       	   var len = steps.length,
       	       i=0;
       	   var timer = setInterval(function(){
               if(i<len){
       	   	      moveFish.css({'top':steps[i].top,'left':steps[i].left}).fadeIn('slow',function(){
       	   	      	 var _clsName = 'fishBubble'+(i+1);
       	   	      	 eduWrap.append('<li class="'+_clsName+'"></li>');
       	   	      	 moveFish.fadeOut('slow',function(){
       	   	      	     $('.'+_clsName).fadeIn('slow');
       	   	      	 });
       	   	      	 i++;
       	   	      });
               }else{
